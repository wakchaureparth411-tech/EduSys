...
export const StateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const savedStudents = localStorage.getItem('edusys_students');
        if (savedStudents) setStudents(JSON.parse(savedStudents));

        const savedTeachers = localStorage.getItem('edusys_teachers');
        if (savedTeachers) setTeachers(JSON.parse(savedTeachers));

        const savedGuards = localStorage.getItem('edusys_guards');
        if (savedGuards) setGuards(JSON.parse(savedGuards));

        const savedAdmins = localStorage.getItem('edusys_admins');
        if (savedAdmins) setAdmins(JSON.parse(savedAdmins));

        const savedGetPass = localStorage.getItem('edusys_getpass');
        if (savedGetPass) setGetPassRequests(JSON.parse(savedGetPass));

        const savedSettings = localStorage.getItem('edusys_settings');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && students.length > 0) {
      localStorage.setItem('edusys_students', JSON.stringify(students));
    }
  }, [students]);

  useEffect(() => {
    if (typeof window !== 'undefined' && teachers.length > 0) {
      localStorage.setItem('edusys_teachers', JSON.stringify(teachers));
    }
  }, [teachers]);

  useEffect(() => {
    if (typeof window !== 'undefined' && guards.length > 0) {
      localStorage.setItem('edusys_guards', JSON.stringify(guards));
    }
  }, [guards]);

  useEffect(() => {
    if (typeof window !== 'undefined' && admins.length > 0) {
      localStorage.setItem('edusys_admins', JSON.stringify(admins));
    }
  }, [admins]);

  useEffect(() => {
    if (typeof window !== 'undefined' && getPassRequests.length > 0) {
      localStorage.setItem('edusys_getpass', JSON.stringify(getPassRequests));
    }
  }, [getPassRequests]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('edusys_settings', JSON.stringify(settings));
    }
  }, [settings]);
  ...
  const deleteStudent = async (id: string) => {
    ...
    if (!res.success) {
      ...
      addNotification(`Student ${student.fullName} registration removed`);
    }
  };
  ...
  const updateTeacher = async (updatedTeacher: Teacher) => {
    const original = teachers.find(t => t.id === updatedTeacher.id);
  ...
  // Guards CRUD
  const addGuard = async (guardData: Omit<Guard, 'id'>) => {
  ...
  const updateGuard = async (updatedGuard: Guard) => {
    const original = guards.find(g => g.id === updatedGuard.id);
