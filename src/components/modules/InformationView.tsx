...
export const InformationView: React.FC = () => {
  ...
  const handleAutoSave = async (updatedFields: any) => {
    const updatedPerson = { ...editForm, ...updatedFields };
    setEditForm(updatedPerson);
    setSelectedPerson(updatedPerson);

    if (!updatedPerson.fullName || updatedPerson.fullName.trim() === '') {
      return;
    }
    ...
    try {
      if (activeTab === 'student') {
        await updateStudent(updatedPerson);
      } else if (activeTab === 'teacher') {
        await updateTeacher(updatedPerson);
      } else if (activeTab === 'guard') {
        await updateGuard(updatedPerson);
      } else if (activeTab === 'admin') {
        await updateAdmin(updatedPerson);
      }
      setSaveStatus('saved');
      // Keep "saved" state visible briefly before returning to idle
              ...
              {!isEditing ? (
                ...
                <form onSubmit={(e) => { e.preventDefault(); setIsEditing(false); }} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          handleAutoSave({ dob: e.target.value });
                        }}
                        className="w-full px-3 py-2 text-xs rounded-lg bg-black/5 dark:bg-white/5 border border-[var(--card-border)] text-zinc-800 dark:text-zinc-200 focus:outline-none"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onBlur={(e) => handleAutoSave({ password: e.target.value })}
                  ...
                  <div className="flex justify-between items-center border-t border-[var(--card-border)] pt-4 mt-2">
                    <div className="text-[11px] font-semibold flex items-center gap-1.5 transition-all duration-300">
                      {saveStatus === 'saving' && (
                        <>
                          ...
                          <span className="text-zinc-500 dark:text-zinc-400">Saving changes...</span>
                        </>
                      )}
                      {saveStatus === 'saved' && (
                        <>
                      ...
                      {saveStatus === 'error' && (
                        <>
                          ...
                          <span className="text-red-500 font-bold">Error saving changes</span>
                        </>
                      )}
                      {saveStatus === 'idle' && (
                        <>
                          <span className="w-2.5 h-2.5 rounded-full bg-zinc-400" />
                          <span className="text-zinc-400">Changes auto-saved instantly</span>
                        </>
                      )}
                    </div>
                    ...
                    <button
                      type="submit"
                      className="px-5 py-2.5 rounded-xl bg-brand-primary text-white text-xs font-bold shadow hover:scale-[1.01] transition-all"
                    >
                      Done
                    </button>
                  </div>
